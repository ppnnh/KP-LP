...
namespace MFST
{
	...
	Mfst::Mfst(LT::LexTable& plexTable, GRB::Greibach pgrebach)
	{
		this->grebach = pgrebach;
		this->lexTable = plexTable;
		this->lenta_size = lexTable.size;
		this->lenta = new short[lexTable.size];

		for (int k = 0; k < this->lenta_size; k++)
			this->lenta[k] = GRB::Rule::Chain::T((GRBALPHABET)lexTable.table[k].lexema[0]);
	...
	Error::ErrorTable::Error Mfst::getError() {
		Error::ErrorTable::Error error;
		int lpos = -1;
		if ((lpos = diagnosis[0].lenta_position) >= 0)
		{
			error.id = grebach.getRule(diagnosis[0].nrule).iderror;
			error.line = lexTable.table[lpos].sn;
			error.pos = 0;
		}
		return error;
	...
	void Mfst::printRules(std::ostream& stream)
	{
		MfstState state;
		GRB::Rule rule;
	...
	bool Mfst::DeleteMfst() {
		delete[] this->deducation.nrules;
		delete[] this->deducation.nrulechains;
		delete[] this->lenta;
